<x-app-layout>
    <x-slot name="header">
        Detail Presensi: {{ $kelas->mataKuliah->nama_mk }}
    </x-slot>

    <div class="mb-6">
        <a href="{{ route('mahasiswa.presensi.index') }}" class="inline-flex items-center gap-2 text-slate-600 hover:text-indigo-600 transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            Kembali ke Rekap Presensi
        </a>
    </div>

    <!-- Mata Kuliah Info -->
    <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-6 text-white mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold">{{ $kelas->mataKuliah->nama_mk }}</h2>
                <p class="opacity-80 mt-1">{{ $kelas->mataKuliah->kode_mk }} • {{ $kelas->mataKuliah->sks }} SKS • Kelas {{ $kelas->nama_kelas }}</p>
                <p class="opacity-60 text-sm mt-2">Dosen: {{ $kelas->dosen->user->name ?? '-' }}</p>
            </div>
            <div class="text-right">
                <div class="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center mb-2">
                    <span class="text-3xl font-bold">{{ $rekap['persentase'] }}%</span>
                </div>
                <p class="opacity-80 text-sm">Kehadiran</p>
            </div>
        </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <!-- Stats Cards -->
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 text-center">
            <div class="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center mx-auto mb-3">
                <svg class="w-6 h-6 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
            </div>
            <p class="text-2xl font-bold text-slate-800">{{ $rekap['hadir'] }}</p>
            <p class="text-sm text-slate-500">Hadir</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 text-center">
            <div class="w-12 h-12 rounded-xl bg-amber-100 flex items-center justify-center mx-auto mb-3">
                <svg class="w-6 h-6 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <p class="text-2xl font-bold text-slate-800">{{ $rekap['sakit'] }}</p>
            <p class="text-sm text-slate-500">Sakit</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 text-center">
            <div class="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center mx-auto mb-3">
                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
            </div>
            <p class="text-2xl font-bold text-slate-800">{{ $rekap['izin'] }}</p>
            <p class="text-sm text-slate-500">Izin</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 text-center">
            <div class="w-12 h-12 rounded-xl bg-red-100 flex items-center justify-center mx-auto mb-3">
                <svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </div>
            <p class="text-2xl font-bold text-slate-800">{{ $rekap['alpa'] }}</p>
            <p class="text-sm text-slate-500">Alpa</p>
        </div>
    </div>

    <!-- Detail Pertemuan -->
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-6 border-b border-slate-100">
            <h3 class="font-bold text-slate-800">Riwayat Presensi per Pertemuan</h3>
        </div>
        
        @if($pertemuanList->isEmpty())
        <div class="p-12 text-center">
            <div class="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-4">
                <svg class="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
            </div>
            <p class="text-slate-500">Belum ada pertemuan yang tercatat</p>
        </div>
        @else
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr class="bg-slate-50">
                        <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Pertemuan</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Tanggal</th>
                        <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Materi</th>
                        <th class="text-center px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @foreach($pertemuanList as $pertemuan)
                    @php
                        $presensi = $presensiData[$pertemuan->id] ?? null;
                        $status = $presensi?->status ?? 'belum';
                        $statusColors = [
                            'hadir' => 'bg-emerald-100 text-emerald-700',
                            'sakit' => 'bg-amber-100 text-amber-700',
                            'izin' => 'bg-blue-100 text-blue-700',
                            'alpa' => 'bg-red-100 text-red-700',
                            'belum' => 'bg-slate-100 text-slate-500',
                        ];
                        $statusLabels = [
                            'hadir' => 'Hadir',
                            'sakit' => 'Sakit',
                            'izin' => 'Izin',
                            'alpa' => 'Alpa',
                            'belum' => 'Belum Dicatat',
                        ];
                    @endphp
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold">
                                    {{ $pertemuan->pertemuan_ke }}
                                </div>
                                <span class="font-medium text-slate-800">Pertemuan {{ $pertemuan->pertemuan_ke }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-slate-600">
                            {{ $pertemuan->tanggal->format('d M Y') }}
                        </td>
                        <td class="px-6 py-4 text-slate-600">
                            {{ $pertemuan->materi ?? '-' }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex px-3 py-1 rounded-full text-sm font-medium {{ $statusColors[$status] }}">
                                {{ $statusLabels[$status] }}
                            </span>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
        @endif
    </div>
</x-app-layout>
